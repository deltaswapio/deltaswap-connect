...
/**
 * The DeltaswapContext manages connections to Deltaswap Core, Bridge and NFT Bridge contracts.
 ...
 * deployments. These can be imported directly from the deltaswap sdk.
 *
 * @example
 ...
 * )
 */
export class DeltaswapContext extends MultiProvider<Domain> {
  private foreignAssetCache: ForeignAssetCache;
  readonly conf: DeltaswapConfig;

  constructor(
    env: Environment,
    conf?: DeltaswapConfig,
    ...
    if (conf) {
      ...
      this.conf = DeltaswapContext.getConfig(env);
    }
  ...
  /**
   * Fetches the address for a token representation on any chain (These are the Deltaswap token addresses, not necessarily the canonical version of that token)
   *
  ...
  async send(
    ...
    if (recipientChain === 'sei') {
      ...
      const { payload: seiPayload, receiver } = await (
        this.getContext('sei') as SeiContext<DeltaswapContext>
   ...
   * @returns A Deltaswap Config
   */
  static getConfig(env: Environment): DeltaswapConfig {
    return env === 'MAINNET'
      ? MAINNET_CONFIG
      : env === 'DEVNET'
      ? DEVNET_CONFIG
