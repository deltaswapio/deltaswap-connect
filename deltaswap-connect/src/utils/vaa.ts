import { getSignedVAA, parseTokenTransferVaa } from '@deltaswapio/deltaswap-sdk';
...
import { CHAINS, ENV, DELTASWAP_API } from 'config';
import {
  ParsedMessage,
...
export async function fetchVaa(
  ...
  if (!vaa) {
    console.warn(
      'Failed to fetch VAA from wormscan. Falling back to phylax.',
    );
    vaa = await fetchVaaPhylax(txData);
  }
...
export async function fetchVaaWormscan(
  ...
  const url = `${DELTASWAP_API}api/v1/vaas/${emitterChain}/${emitterAddress}/${sequence}`;
    ...
    .then(function (response: any) {
      ...
      const vaaData: ParsedVaa = {
        ...
        fromAddress: parsed.fromAddress
          ...
          : undefined,
        phylaxSignatures: parsed.phylaxSignatures.length,
...
export async function fetchVaaPhylax(
  txData: ParsedMessage | ParsedRelayerMessage,
): Promise<ParsedVaa | undefined> {
  // return if the number of block confirmations hasn't been met
  const chainName = wh.toChainName(txData.fromChain);
...
export const fetchIsVAAEnqueued = async (
  ...
  const url = `${DELTASWAP_API}v1/governor/is_vaa_enqueued/${emitterChain}/${emitterAddress}/${sequence}`;
...
export const fetchGlobalTx = async (
  ...
  const url = `${DELTASWAP_API}api/v1/global-tx/${emitterChain}/${emitterAddress}/${sequence}`;
  return axios
    .get(url)
    .then(function (response: any) {
      const data = response.data;
