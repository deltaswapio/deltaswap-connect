...
export class CCTPRelayRoute extends CCTPManualRoute implements RelayAbstract {
  ...
  async estimateSendGas(
    ...
    const chainContext = wh.getContext(
      sendingChain,
    ) as EthContext<DeltaswapContext>;
    const circleRelayer =
      chainContext.contracts.mustGetDeltaswapCircleRelayer(sendingChain);
  ...
  async redeem(
    ...
    // TODO: implement redeemRelay in the DeltaswapContext for self redemptions
  ...
  async getRelayerFee(
    ...
    const circleRelayer =
      chainContext.contracts.mustGetDeltaswapCircleRelayer(sourceChain);
  ...
  async nativeTokenAmount(
    ...
    const relayer = context.contracts.mustGetDeltaswapCircleRelayer(destChain);
...
async function fetchSwapEvent(txData: ParsedMessage | ParsedRelayerMessage) {
  ...
  const chainName = wh.toChainName(txData.toChain) as ChainName;
  const chainConfig = CHAINS[chainName]!;
  const relayerContract = context.contracts.mustGetDeltaswapCircleRelayer(
    txData.toChain,
  );
