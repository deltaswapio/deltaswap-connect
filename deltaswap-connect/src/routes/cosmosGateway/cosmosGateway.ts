...
export class CosmosGatewayRoute extends BaseRoute {
  ...
  private async manualRedeem(
    ...
    if (!isSignedDeltaswapMessage(message))
  ...
  async redeem(
    ...
    if (isGatewayChain(chain)) {
      return this.manualRedeem(CHAIN_ID_DELTACHAIN, messageInfo, recipient);
    }
  ...
  async getSignedMessage(
    ...
    // for the first ibc transfer, before it reaches deltachain, so at that time there is no VAA info available
    // so at this point we have to query deltachain to check if the IBC transfer was relayed and the contract was called
    const signedMessage = isGatewayChain(unsignedMessage.fromChain)
      ? await getMessageFromDeltachain(
    ...
    const vaa = await fetchVaa({
      ...signedMessage,
      // transfers from cosmos vaas are emitted by deltachain and not by the source chain
