...
export async function getUnsignedMessageFromCosmos(
  ...
  const base = await adaptParsedMessage({
    ...
    payloadID: 1, // no vaa, but deltachain will eventually emit a normal transfer
    tokenChain,
    tokenAddress,
    tokenId: {
...
async function getOriginalIbcDenomInfo(
  ...
  const context = wh.getContext(
    CHAIN_ID_DELTACHAIN,
  ) as CosmosContext<DeltaswapContext>;
  const { chainId, assetAddress: tokenAddressBytes } =
    await context.getOriginalAsset(CHAIN_ID_DELTACHAIN, cw20);
  const tokenChain = wh.toChainName(chainId as ChainId); // deltaswap-sdk adds 0 (unset) as a chainId
...
function factoryToCW20(denom: string): string {
  ...
  if (!encoded) return '';
  return cosmos.humanAddress('deltaswap', base58.decode(encoded));
}

export async function getMessageFromDeltachain(
  hash: string,
  chain: ChainName,
): Promise<UnsignedMessage> {
  // Get tx on the source chain (e.g. osmosis)
  ...
  // Look for the matching IBC receive on deltachain
  ...
  if (!destTx) {
    throw new Error(
      `No deltachain transaction found for packet on chain ${chain}`,
    );
  }

  const message = await getDeltachainEmittedMessage(destTx.hash, ibcInfo);
  const parsed = await adaptParsedMessage(message);

  return {
    ...parsed,
    // add the original source chain and tx hash to the info
    // the vaa contains only the deltachain information
...
/**
 * Parse a deltachain tx to extract the VAA information
 *
 * @param id Deltachain transaction id
 * @param sourceTxIbcInfo IBC packet information of the source transaction
 * @returns
 */
async function getDeltachainEmittedMessage(
  id: string,
