...
import { DeltaswapGatewayIdl } from './DeltaswapGatewayIdl';
import {
...
export async function sendTbtc(
  ...
  const program = new Program(DeltaswapGatewayIdl, gatewayProgramId, {
    connection,
  });
  ...
  const coreBridgeData = deriveDeltaswapBridgeDataKey(coreBridgeProgramId);
