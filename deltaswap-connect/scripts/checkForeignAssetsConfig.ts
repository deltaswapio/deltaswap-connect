...
const DELTACHAIN_ERROR_MESSAGES = [
  '3104 RPC not configured',
  'deltachain RPC not configured',
...
const checkEnvConfig = async (
  ...
  const wh = new DeltaswapContext(env);
        ...
        return (async () => {
          ...
          if (chain === tokenConfig.nativeChain) {
            ...
            try {
              ...
              if (DELTACHAIN_ERROR_MESSAGES.includes(e?.message)) {
                // do not throw on deltachain errors
              } else {
                throw e;
              }
            }
